...
export default function DayColumn({ dayIndex, entries, onRemoveEntry, onRepeatEntry }: DayColumnProps) {
  ...
  return (
    <div className={`flex flex-col min-w-0 h-full ${isWeekend ? 'bg-orange-50/40' : ''} rounded-lg`}>
      ...
      {/* Morning slot */}
      <div className="px-0.5 md:px-1.5 pb-0.5 flex-1 flex flex-col">
        <TimeSlot
          dayIndex={dayIndex}
          timeSlot="morning"
      ...
      {/* Lunch divider */}
      <div className="lunch-divider mx-0.5 md:mx-1.5 py-0.5 md:py-1.5 text-center rounded">
        <span className="text-[10px] md:text-sm text-orange-400">🍽😴</span>
      </div>

      {/* Afternoon slot */}
      <div className="px-0.5 md:px-1.5 pt-0.5 flex-1 flex flex-col">
        <TimeSlot
          dayIndex={dayIndex}
          timeSlot="afternoon"
