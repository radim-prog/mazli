...
export default function PlacedActivity({ activity, entryId, onRemove, onRepeat }: PlacedActivityProps) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className="activity-tile flex-1 flex flex-col items-center justify-center gap-0.5 md:gap-1 px-1 py-1 md:px-2 md:py-3 rounded-xl group relative cursor-grab active:cursor-grabbing select-none min-h-0"
      style={style}
    >
      <span className="text-2xl md:text-4xl" role="img">{activity.emoji}</span>
      <span className="font-semibold text-gray-700 text-[10px] md:text-base text-center leading-tight truncate w-full">{activity.name}</span>
